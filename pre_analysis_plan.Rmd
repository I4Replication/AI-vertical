...
Key design features at a glance: (i) stratified randomization within five expertise tiers (UG, MA, PhD, PD, P); (ii) 1:1 allocation to AI‑assisted vs human‑only arms; (iii) discipline‑tagged tasks that allow an inside/outside‑discipline (OOD) contrast; (iv) two primary families of estimands—vertical (AI × tier) and horizontal (AI × OOD)—with multiplicity controlled separately in each family using Holm’s method at α = 0.05; (v) blinded grading for human referee outcomes. Communication quality is operationalized via two preregistered “referee” outcomes: an Appropriateness indicator (binary) and a 0–5 Score anchored to a rubric (Section “Grading Rubric”).
...
Primary families and question framing:
- Vertical (primary): Does AI compress the expertise gradient? Test via AI × tier interactions (equalizer vs amplifier) on all primary outcomes.
- Horizontal (primary): Does AI reduce the outside‑of‑discipline (OOD) penalty? Test via AI × OOD interactions on all primary outcomes.
- Average effect (supporting): Does AI increase success and reduce time‑to‑success on average (ITT), irrespective of heterogeneity?

Hypotheses (formatted):
- H1 — Average effect: AI increases reproduction success and reduces time‑to‑success (ITT).
- H2 — Vertical compression: AI gains are weakly larger for lower tiers (UG/MA) than higher tiers (PD/P), reducing tier gaps.
- H2b — Horizontal compression: AI reduces the OOD penalty (AI × OOD improves success, reduces minutes, and aligns error‑detection patterns when outside discipline).
- H3 — Error detection: AI increases detection of major and minor coding errors.

Key terms (definitions used throughout):
- Reproduction success: Final output matches the pre‑specified focal result within documented tolerance (numeric or visual match); adjudicated via rubric.
- Time‑to‑success: Minutes from start until first successful reproduction; right‑censored at 420 minutes for non‑successes.
- Error detection: Count of correctly identified coding errors, split into Major (affects focal result’s sign/magnitude or uses wrong data/model/code path) vs Minor (formatting or non‑substantive code issues not affecting the focal result); examples in Grading Rubric.
- Communication quality: Referee appropriateness (binary) and a 0–5 score anchored to clarity, correctness, and justification of claims.
- Out‑of‑discipline (OOD): Task’s discipline differs from participant’s primary discipline (Economics, Political Science, Psychology).
...
Error detection
- Major error (count): Any coding/data/model issue that changes the focal result’s sign, statistical significance, or interpretation, or that uses a wrong dataset/model/code path. Examples: wrong sample filter; omitted transformation altering effect size.
...
- Score (0–5 anchors): 0=No assessment; 1=Poor/incorrect; 2=Limited/partially incorrect; 3=Adequate/correct but thin; 4=Strong/correct and well‑justified; 5=Exceptional/insightful with clear justification and actionable suggestions.
...
We plan six one‑day events across partner institutions within the academic year, spaced to avoid overlap and allow consistent staffing. Events are pre‑announced with a shared protocol: pre‑generated randomization within tiers, a standardized onboarding for the AI arm, and a seven‑hour work window. Each individual may participate in at most one event; duplicate registrations are blocked at check‑in. We lock randomization and materials ahead of time and document any deviations (no‑shows, substitutions) prior to analysis.

Event and article selection. Events are hosted by partner institutions with capacity for proctoring and secure data handling. Articles are selected from the I4R pipeline to span the three disciplines and software ecosystems (R/Stata/Python). Selection prioritizes clarity of a focal result and a mix of tasks with and without known coding pitfalls; each article is tagged to a single discipline before the lock. We aim for temporal balance across the academic year to minimize tool‑version drift; any updates to AI tooling are logged.
